...
Capacitor wraps the web build: `npm run android:apk` builds the bundle, syncs it into `android/`, and produces `android/app/build/outputs/apk/debug/app-debug.apk` (debug-signed, portrait-locked, `com.ngg.smallacts`). Requires JDK 21 and an Android SDK with platform 36 and build-tools 36.0.0; point `android/local.properties` at the SDK (`sdk.dir=...`, untracked).
