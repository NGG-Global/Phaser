...
`game/session.ts` is the content playlist: rounds of `{ vignette, tasks }`, flattened by `sessionTasks()`; consecutive rounds must change vignette. The host advances one task at a time, slides between tasks of a round, paints a transition only at round boundaries, and shows a minimal summary after the last task. `MenuScene` precedes it and owns the audio gesture. It reuses `TaskSequence.ending()` for absolute coda/slide/swap/downbeat times, and `RoundController` still owns each task's timing and scoring. Session difficulty is content, not special cases in the controller. No clock, input, scheduler or judgement rules changed for this milestone.
